...
void (async () => {
  ...
  const updated = setRevocationStatus(state, commitment, true);
  ...
  console.log("Revoked attestation commitment:", commitment);
  ...
  console.error("Registry attestation revoke failed.");
  console.error(error);
  process.exit(1);
});
